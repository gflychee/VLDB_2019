...
struct ConnectionCountsCacheElement {
	...
	ConnectionCountsCacheElement(): times_accessed(0), num_samples(-1),
		counts(std::vector<size_t>()) {
		// It's here only to satisfy the compiler
		throw std::logic_error("Never use this constructor");
	}

	ConnectionCountsCacheElement(size_t n): times_accessed(0), num_samples(-1),counts(std::vector<size_t>(n)) {}
